...
function Score3Badge({ row }: { row: EarningsQualityRow }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full font-mono text-caption font-medium tabular-nums"
      style={{ backgroundColor: bg, color: text, border: `1px solid ${border}` }}
      title={title}
    >
      {row.score3}
      <span className="text-caption opacity-60">/{max}</span>
      {isLegacy && <span className="text-caption font-normal opacity-70">旧</span>}
    </span>
  )
}
...
function DivCell({ row }: { row: EarningsQualityRow }) {
  ...
  const main = !isNum(v) ? (
    <span className="text-[var(--text-muted)] text-caption" title={scoreLabel}>
      —
    </span>
  ) : (
    <span
    ...
    >
      {v > 0 ? '+' : ''}
      {v.toFixed(1)}%
      {v >= 10 && <span className="ml-0.5 text-caption">★</span>}
    </span>
  )
...
function OpCell({ row }: { row: EarningsQualityRow }) {
  if (row.cur_per_type === 'FY') {
    ...
    if (!isNum(row.op_beat_pct)) {
      return (
        <span className="text-[var(--text-muted)] text-caption" title={`FY は通期予想修正の対象外${nx}`}>
          —
        </span>
      )
    }
  ...
  return (
    ...
    >
      ...
      {guideScored && <span className="ml-0.5 text-caption text-[var(--positive)]">+{row.s_guide}</span>}
    </span>
  )
}
...
function ProgressCell({ row }: { row: EarningsQualityRow }) {
  ...
  if (!isNum(v)) {
    return row.cur_per_type === 'FY' ? (
      <span className="text-[var(--text-muted)] text-caption" title="FY (通期本決算) は進捗の概念が無いため対象外">
        対象外
      </span>
    ) : (
  ...
  return (
    <span className="font-mono tabular-nums text-caption" style={{ color: pctColor(v) }}>
...
function DiscTimeCell({ t }: { t: string | null }) {
  if (!t) return <span className="text-[var(--text-muted)] text-caption">—</span>
  const afterClose = isAfterClose(t)
  return (
    <span
      className={`font-mono text-caption ${afterClose ? 'font-medium text-[var(--sem-watch-fg)]' : 'text-[var(--text-secondary)]'}`}
      title={afterClose ? '引け後開示 → 翌営業日 (D+1) 寄り対象' : undefined}
    >
      {t}
      {afterClose && <span className="ml-0.5 text-caption">⏰</span>}
    </span>
  )
}
...
export default function EarningsQualitySection({
  ...
  return (
    <>
      ...
      {/* ── 旧スコア (v2 バックフィル前) の注意 ─────────────────────── */}
      {scoreData.state !== 'ok' && (
        <div className="mb-5 px-4 py-2.5 rounded-lg bg-[var(--sem-watch-bg)] border border-[var(--sem-watch-bd)] text-[var(--sem-watch-fg)] text-small flex items-start gap-2">
          <div>
            {scoreData.state === 'column-missing' ? (
              <>
                <p className="font-medium">スコアの内訳 (s_guide) が取得できません</p>
                <p className="text-caption text-[var(--sem-watch-fg)] mt-0.5">
              ...
              <>
                <p className="font-medium">この日は旧スコア (0-7 / 3軸) のままです</p>
                <p className="text-caption text-[var(--sem-watch-fg)] mt-0.5">
      ...
      {isPeakDay && (
        <div className="mb-5 px-4 py-2.5 rounded-lg bg-[var(--sem-focus-bg)] border border-[var(--sem-focus-bd)] text-[var(--sem-focus-fg)] text-small flex items-center gap-2">
          <span className="text-lead">🔥</span>
          <span className="font-medium">集中日 (ピーク)</span>
          <span className="text-caption text-[var(--sem-focus-fg)]">
        ...
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-4 mb-5 space-y-3">
          ...
          {qOptions.length > 1 && (
            <div className="flex flex-wrap items-start gap-2">
              <span className="text-caption font-medium text-[var(--text-secondary)] uppercase w-14 shrink-0 mt-1">
                Q
              </span>
              <div className="flex flex-wrap gap-1.5 flex-1">
                ...
                {qSelected.size > 0 && (
                  <button
                    ...
                    className="text-caption text-[var(--text-muted)] hover:text-[var(--text-secondary)]"
                  >
                    クリア
                  </button>
                )}
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm overflow-x-auto">
        <div className="flex items-center gap-3 px-4 pt-4 pb-3 flex-wrap">
          <p className="text-small font-medium text-[var(--text-primary)]">品質スコア ランキング</p>
          {hasDup && (
            <label
              className="flex items-center gap-1.5 text-caption text-[var(--text-secondary)] cursor-pointer select-none"
        ...
        {/* Legend */}
        <div className="flex items-center justify-center gap-4 py-3 text-caption border-t border-[var(--border-subtle)] flex-wrap">
          ...
          >
            <span className={`px-1.5 rounded-full text-caption font-mono font-medium ${qBadgeClass('FY')}`}>
              FY
            </span>
            1Q・FY は上限 7 点
          </span>
          ...
          {hasDup && (
            <>
              ...
              >
                <span className="px-1 rounded text-caption font-medium bg-[var(--bg-primary)] text-[var(--text-secondary)]">
                  同日N
                </span>
                同一銘柄が同日に N 本開示
              </span>
...
function StatCard({
  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-3">
      <p className="text-caption text-[var(--text-secondary)] uppercase tracking-wide font-medium">{label}</p>
      <p className={`text-title font-medium font-mono tabular-nums mt-1 ${accentColor}`}>{value}</p>
      {sub && <p className="text-caption text-[var(--text-muted)] mt-0.5 truncate" title={sub}>{sub}</p>}
    </div>
  )
}
