...
- 本体 2 セクション
  1. **CurrentStateTable**（`Current State`）— `watchlist_current` を state でグループ化
     （`HOLD → READY → FOCUS → SECOND → SHORT → OTHERS → INBOX → SOLD`）。既定ソートは滞在日数の降順。
     `SOLD`（売却済アーカイブ）は既定で折りたたむ。列は 15 列:
     `Code / Name` `Sector` `Since` `Days` `Entry → Now` `Return` `ADR %` `ATR14`
...
- 列見出しとセクション名は英語、説明ツールチップは日本語（repo の表記規約どおり）
- **21EMA 系リスク列**（2026-09-05 に両テーブルへ 7 列追加）: `1R` は安値 21EMA にストップを置いた
  ときの値幅で、TradingView の Cockpit BT10 / ADR×ATR と同じ値。円建ては呼値に丸めていない生値なので
  TV 表示と数円ずれる（照合は % 側で行う。% は小数第 2 位まで一致）。`1R %` が 8 以上は
  RR2:1 が成立せずエントリー対象外だが、**画面では色も印も付けない**（判断は数値を見て手で行う）
- **勝率・PF・期待値は出さない**（サンプル過少。集計は中央値までで、件数を明記）
- `scanner_names` は両テーブルとも非表示（空の意味が「スキャナーが拾えなかった」と
  「その日リストを貼らなかった」で区別できず、読み手を誤らせるため）
